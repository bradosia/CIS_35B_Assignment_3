...
public class AutomobileTable {
	...
	public String insertOverwrite(Automobile automobileObject) {
		/* key = Make-Model-Year */
		String automobileKey = automobileObject.getMake() + "-" + automobileObject.getModel() + "-"
				+ automobileObject.getYear();
		automobileTable.put(automobileKey, automobileObject);
		return automobileKey;
	}

	/*
	 * Inserts an automobile into the hash table. If an automobile with the same key
	 * exists then the exception fixString() method is used to correct it.
	 * 
	 * @return the key in the hash table
	 */
